...
db.serialize(() => {
    db.run("UPDATE Clubs SET location = REPLACE(location, '語言大樓', '語文大樓')");
    db.run("UPDATE Events SET location = REPLACE(location, '語言大樓', '語文大樓')");
    ...
    db.all("SELECT id, location FROM Clubs", (err, rows) => {
        ...
        console.log("Clubs structure update complete.");
    });
    ...
    db.all("SELECT id, location FROM Events", (err, rows) => {
        ...
        console.log("Events structure update complete.");
    });
});
